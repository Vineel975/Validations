...
- icdCode1: ICD-10 code Level 1 — the primary/most specific ICD-10 code for the main diagnosis. ALWAYS return a value — extract directly from the document if printed, otherwise DERIVE from the diagnosis text. Examples: "Cataract" → "H26.9", "Cataract right eye" → "H25.11", "Appendicitis" → "K37", "Cholecystitis" → "K81.9". Never return null for icdCode1 — always derive a best-match code from the diagnosis.
- icdCode2: ICD-10 code Level 2 — a secondary ICD-10 code for a related condition, complication, or comorbidity mentioned in the document. If there is only one diagnosis, use a closely related complication or the procedural ICD code. Return only the code string. Return null only if truly no secondary condition exists.
- icdCode3: ICD-10 code Level 3 — a tertiary ICD-10 code for any additional condition, procedure code, or finding. If fewer than 3 conditions, use the procedure ICD-10-PCS code or a symptom code related to the diagnosis. Return null only if nothing applicable.
